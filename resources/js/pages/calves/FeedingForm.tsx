        ...
        <div className="px-4 py-4 space-y-4 pb-8">
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1">
              Calf <span className="text-red-500">*</span>
            </label>
            {calves.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                value={data.fed_on}
                max={today()}
                onChange={e => setData('fed_on', e.target.value)}
                className="h-12 w-full rounded-xl border border-gray-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600"
              />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {FEED_TYPES.map(ft => (
                <button
                  key={ft.value}
                  type="button"
                  onClick={() => handleFeedChange(ft.value)}
                  className={clsx(
                    'py-3 px-3 rounded-xl border text-sm font-medium text-left transition-all',
          ...
          <Input
            ...
            inputMode="decimal"
            placeholder="4"
            unit={selectedFeedType?.unit ?? 'litres'}
            value={data.quantity}
          ...
          {data.feed_type === 'whole_milk' && lactatingCows.length > 0 && (
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">
                Milk Source Cow (optional)
              </label>
              <select
                value={data.milk_source_animal_id}
                onChange={e => setData('milk_source_animal_id', e.target.value)}
                className="h-12 w-full rounded-xl border border-gray-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600"
              >
