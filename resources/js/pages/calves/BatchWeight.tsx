...
export default function BatchWeight() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await axios.post('/weight', { entries: payload });
      setSaved(true);
      setTimeout(() => router.visit('/calves'), 1500);
    } catch {
      // fallback to form submit
      router.post('/weight', { entries: payload });
    } finally {
      setSaving(false);
    }
  };
          ...
          return (
            <div key={animal.id} className="bg-white border border-gray-100 rounded-xl p-3">
              <div className="grid grid-cols-[1fr_90px_70px] gap-2 items-center">
                ...
                {/* Weight input */}
                <input
                  type="number" min="5" max="1200" step="0.5" inputMode="decimal"
                  placeholder={lastRec ? lastRec.weight_kg.toString() : '0'}
                ...
                {/* BCS input */}
                <input
                  type="number" min="1" max="5" step="0.5" inputMode="decimal"
                  placeholder="3.0"
              ...
              {entry?.weight_kg && lastRec && parseFloat(entry.weight_kg) > 0 && lastRec.measured_on && (
                <div className="mt-2 pt-2 border-t border-gray-100">
                  <AdgPreview
                    ...
                    oldWeight={parseFloat(lastRec.weight_kg.toString())}
