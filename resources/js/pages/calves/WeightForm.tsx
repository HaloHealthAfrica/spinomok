      ...
      <form onSubmit={submit} noValidate>
        <div className="px-4 py-4 space-y-4">
          ...
          <div>
            ...
            <select value={data.entries[0].animal_id} onChange={e => updateEntry('animal_id', e.target.value)} required
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1">Date Measured <span className="text-red-500">*</span></label>
            <input type="date" value={data.entries[0].measured_on} max={today()}
              onChange={e => updateEntry('measured_on', e.target.value)}
              className="h-12 w-full rounded-xl border border-gray-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600" />
          </div>

          <Input label="Weight" type="number" min="5" max="1200" step="0.5" inputMode="decimal"
            placeholder="45" unit="kg" required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Body Condition Score</label>
              <div className="relative">
                <input type="number" min="1" max="5" step="0.5" inputMode="decimal"
                  placeholder="3.0"
            ...
            <div>
              ...
              <select value={data.entries[0].method} onChange={e => updateEntry('method', e.target.value)}
                className="h-12 w-full rounded-xl border border-gray-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600">
          ...
          <div>
            ...
            <textarea value={data.entries[0].notes} onChange={e => updateEntry('notes', e.target.value)} rows={2}
