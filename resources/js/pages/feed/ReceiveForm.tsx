...
export default function ReceiveForm() {
  ...
  const handleFeedChange = (id: string) => {
    ...
    setData(d => ({ ...d, feed_type_id: id, unit_cost: lastCost ? String(lastCost) : d.unit_cost }));
  };
  ...
  return (
    <AppLayout title="Receive Feed Stock">
      ...
      <form onSubmit={submit} noValidate>
        <div className="px-4 py-4 space-y-4">
            ...
            <div className="flex gap-2">
              {[
                ...
                <button
                  key={t.value}
                  type="button"
                  onClick={() => setData('transaction_type', t.value)}
                  className={clsx(
                    'flex-1 py-2.5 rounded-xl border text-sm font-medium transition-all',
                    data.transaction_type === t.value
                      ? 'bg-primary-900 text-white border-primary-900'
                      : 'bg-white text-gray-600 border-gray-300',
                  )}
                >
                  {t.value === 'purchase' ? '🛒 ' : '🌿 '}{t.label}
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            >
              {['roughage', 'concentrate', 'silage', 'hay', 'mineral', 'supplement', 'other'].map(cat => {
          ...
          {totalCost && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-3">
              <p className="text-sm font-bold text-green-900">
                Total Value: KES {parseFloat(totalCost).toLocaleString()}
              </p>
              <p className="text-xs text-green-600">
                {data.quantity_kg} kg x KES {data.unit_cost}/kg
              </p>
            </div>
          )}
          ...
          {data.transaction_type === 'purchase' && (
            <div className="grid grid-cols-2 gap-3">
              <Input
                ...
                placeholder="e.g. Nakuru Feeds Ltd"
                value={data.supplier}
                onChange={e => setData('supplier', e.target.value)}
              />
              <Input
                label="Invoice / LPO No."
          ...
          >
            ...
            {data.quantity_kg ? ` — ${data.quantity_kg} kg` : ''}
          </Button>
        </div>
      </form>
    </AppLayout>
